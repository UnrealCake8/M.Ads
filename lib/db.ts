...
export async function listSites(): Promise<Site[]> {
  const rows = await rest<Row[]>("mads_sites?select=*&order=created_at.desc");
...
export async function getMetrics() {
  const [impressions, clicks] = await Promise.all([
    rest<Row[]>("mads_events?select=id&type=eq.impression"),
    rest<Row[]>("mads_events?select=id&type=eq.click"),
  ]);
  return {
